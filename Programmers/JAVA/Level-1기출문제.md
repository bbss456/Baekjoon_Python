...
class Solution {
    public int solution(int[] nums) {
        int answer = 0;
        boolean[] numberBoolean = new boolean[2997];

        /* 소수 구하기 */
        for(int i=0; i < numberBoolean.length; i++) {
            numberBoolean[i] = true;
        }
        numberBoolean[0] = true;
        numberBoolean[1] = false; 
        
        for(int i=2; i<numberBoolean.length; i++) {
            for(int j=i*i; j<numberBoolean.length; j=j+i) {
                numberBoolean[j] = false;
            }
        }
